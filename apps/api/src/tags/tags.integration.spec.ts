...
describe("Cycle 2 tags API", () => {
  ...
  it("normalizes tag names for uniqueness within an organization but allows the same name in another organization", async () => {
    const first = await createOrganizationAdmin({
      organizationName: "Tags Organization A",
      organizationSlug: "tags-org-a",
      email: "tags-a@example.test",
      password: "Strong-Tags-A-Password-2026!",
    });
    const second = await createOrganizationAdmin({
      organizationName: "Tags Organization B",
      organizationSlug: "tags-org-b",
      email: "tags-b@example.test",
      password: "Strong-Tags-B-Password-2026!",
    });

    const created = await request(app.getHttpServer())
      .post("/api/v1/tags")
      .set("Authorization", `Bearer ${first.token}`)
      .set("x-request-id", "cycle2-tag-create-a")
      .send({ name: "  Prioridade VIP  " })
      .expect(201);

    expect(created.body).toMatchObject({
      name: "Prioridade VIP",
      normalizedName: "prioridade vip",
    });
  ...
  it("rejects cross-organization tag links for companies and contacts", async () => {
    ...
    const organizationA = await prisma.organization.create({
      data: { name: "Tag Isolation A", slug: "tag-isolation-a" },
    });
    const organizationB = await prisma.organization.create({
      data: { name: "Tag Isolation B", slug: "tag-isolation-b" },
    });
    const user = await createUser({
      email: "tag-isolation@example.test",
      password,
      displayName: "Tag Isolation Admin",
    });
    ...
    await request(app.getHttpServer())
      .post(`/api/v1/companies/${companyB.id}/tags/${tagA.id}`)
      .set("Authorization", `Bearer ${tokenA}`)
      .expect(404);
    await request(app.getHttpServer())
      .post(`/api/v1/companies/${companyA.id}/tags/${tagB.id}`)
      .set("Authorization", `Bearer ${tokenA}`)
      .expect(404);
    await request(app.getHttpServer())
      .post(`/api/v1/contacts/${contactB.id}/tags/${tagA.id}`)
      .set("Authorization", `Bearer ${tokenA}`)
      .expect(404);
    await request(app.getHttpServer())
      .post(`/api/v1/contacts/${contactA.id}/tags/${tagB.id}`)
      .set("Authorization", `Bearer ${tokenA}`)
      .expect(404);
  ...
  it("links and unlinks tags in the active organization and records audit actions", async () => {
    const { organization, user, token } = await createOrganizationAdmin({
      organizationName: "Tag Lifecycle",
      organizationSlug: "tag-lifecycle",
      email: "tag-lifecycle@example.test",
      password: "Strong-Tag-Lifecycle-2026!",
    });

    const [company, contact, tag] = await Promise.all([
      prisma.withTenant(organization.id, tenant =>
        tenant.company.create({
          data: {
            organizationId: organization.id,
            legalName: "Empresa com Tag",
            createdBy: user.id,
            updatedBy: user.id,
          },
        })
    ...
    await request(app.getHttpServer())
      .post(`/api/v1/companies/${company.id}/tags/${tag.id}`)
      .set("Authorization", `Bearer ${token}`)
      .set("x-request-id", "cycle2-company-tag-link")
      .expect(201);
    await request(app.getHttpServer())
      .post(`/api/v1/contacts/${contact.id}/tags/${tag.id}`)
      .set("Authorization", `Bearer ${token}`)
      .set("x-request-id", "cycle2-contact-tag-link")
      .expect(201);
    ...
    await request(app.getHttpServer())
      .delete(`/api/v1/companies/${company.id}/tags/${tag.id}`)
      .set("Authorization", `Bearer ${token}`)
      .set("x-request-id", "cycle2-company-tag-unlink")
      .expect(204);
    await request(app.getHttpServer())
      .delete(`/api/v1/contacts/${contact.id}/tags/${tag.id}`)
      .set("Authorization", `Bearer ${token}`)
      .set("x-request-id", "cycle2-contact-tag-unlink")
      .expect(204);

    const audit = await prisma.auditLog.findMany({
      where: {
        organizationId: organization.id,
        action: { in: ["tag.linked", "tag.unlinked"] },
      },
