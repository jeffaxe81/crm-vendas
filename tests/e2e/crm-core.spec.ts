...
test("CRM core journey persists company contact channel link and history", async ({
  ...
  await expect(contactCard.getByText(contactEmail, { exact: true })).toBeVisible();
  ...
  await contactCard
    ...
    .click();
  const historyForm = page.getByRole("form", { name: "Registrar histórico" });
  ...
  await expect(contactCard.getByText(relationshipNote, { exact: true })).toBeVisible();

  await page.reload();

  await expect(
    page.getByRole("button", { name: "Contatos" })
  ).toBeVisible();
  await page.getByRole("button", { name: "Contatos" }).click();

  const persistedCard = page.locator("article").filter({ hasText: contactName });
  await expect(persistedCard).toBeVisible();
  await expect(persistedCard.getByText(contactEmail, { exact: true })).toBeVisible();
  await expect(
